...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Roast Name Service", "ROAST"){
    ...
    function registerDomain(string calldata name) public payable {
       if (domainList[name] != address(0)) revert AlreadyRegistered();
   ...
   function getOwner(string calldata name) public view returns(address) {
       return domainList[name];
   }

   function setRecord(string calldata name, string calldata record) public {
       if (msg.sender != domainList[name]) revert Unauthorized();
       records[name] = record;
   }
